    ...
    <div className={`relative w-full ${className}`}>
      ...
      >
        {items.map((item, index) => (
          ...
          >
            <div className={`rounded-lg p-4 ${selectedIndex === index ? activeItemClassName : ''} ${itemClassName}`}>
              {renderItem ? renderItem(item, index, selectedIndex === index) : <p className="m-0 text-white">{item}</p>}
            </div>
          </AnimatedItem>
        ))}
      </div>
